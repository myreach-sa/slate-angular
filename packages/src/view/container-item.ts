...
> {
  ...
  getRootNodes(): HTMLElement[] {
    if (this.embeddedViewRef) {
      return this.embeddedViewRef.rootNodes.filter((rootNode) =>
        isDOMElement(rootNode)
      );
    }
  ...
  createView() {
    ...
    if (isTemplateRef(this.viewType)) {
      ...
      const embeddedViewRef = this.viewContainerRef.createEmbeddedView<
        BaseEmbeddedView<T>
      >(this.viewType, this.embeddedViewContext);
    ...
    if (this.viewType === viewType) {
      ...
      if (isTemplateRef(this.viewType)) {
        ...
        firstRootNode.replaceWith(
          ...embeddedViewRef.rootNodes.filter((rootNode) =>
            isDOMElement(rootNode)
          )
        );
        this.destroyView();
