...
export class SlateDescendantComponent
  extends ViewContainerItem<
    SlateElementContext | SlateTextContext,
    BaseElementComponent | BaseTextComponent
  >
  ...
  createBlockCard() {
    ...
    this.blockCardComponentRef = this.viewContainerRef.createComponent<
      SlateBlockCardComponent
    >(componentFactory);
  ...
  updateBlockCard() {
    ...
    firstRootNode.replaceWith(
      this.blockCardComponentRef.instance.nativeElement
    );
  }
  ...
  getContext(): SlateElementContext | SlateTextContext {
    if (Element.isElement(this.descendant)) {
      ...
      const key = AngularEditor.findKey(
        this.viewContext.editor,
        this.descendant
      );
      ...
      const computedContext = this.getCommonContext();
      const isLeafBlock = AngularEditor.isLeafBlock(
        this.viewContext.editor,
        this.context.parent
      );
      const textContext: SlateTextContext = {
        ...
        isLast:
          isLeafBlock && this.index === this.context.parent.children.length - 1,
  ...
  getViewType(): ViewType {
    if (Element.isElement(this.descendant)) {
      return (
        (this.viewContext.renderElement &&
          this.viewContext.renderElement(this.descendant)) ||
        this.defaultElementComponentType
      );
    } else {
      const isVoid = this.viewContext.editor.isVoid(
        this.context.parent as Element
      );
      return isVoid
        ? SlateVoidTextComponent
        : (this.viewContext.renderText &&
            this.viewContext.renderText(this.descendant)) ||
  ...
  memoizedContext(
    ...
    if (Element.isElement(this.descendant)) {
      return this.memoizedElementContext(
        prev as SlateElementContext,
        next as SlateElementContext
      );
    } else {
      return this.memoizedTextContext(
        prev as SlateTextContext,
        next as SlateTextContext
      );
    }
  }
}
