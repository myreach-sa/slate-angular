...
export class Editable2Component implements OnInit, OnChanges {
  ...
  private readonly scheduleOnDOMSelectionChange = () => {
    this._scheduleOnDOMSelectionChangeTimer = window.setTimeout(() => {
  ...
  ngOnInit(): void {
    ...
    this.androidInputManager = useAndroidInputManager(this.editor, {
      ...
      onDOMSelectionChange: this.onSelectionChangeHandler,
  ...
  private _onSelectionChangeHandlerTimer: number;
  private _onSelectionChangeHandlerLastTimer = performance.now();
  private readonly _onSelectionChangeHandlerThrottle = 100;

  private onSelectionChangeHandlerFlush(): void {
  ...
  public onSelectionChangeHandler(): void {
    ...
    const msPassed = now - this._onSelectionChangeHandlerLastTimer;
    const msUntilNextAvailable =
      this._onSelectionChangeHandlerThrottle - msPassed;
    // not enough time passed
    if (msUntilNextAvailable > 0) {
      this._onSelectionChangeHandlerTimer = window.setTimeout(() => {
        this.onSelectionChangeHandler();
      }, msUntilNextAvailable);
    } else {
      this._onSelectionChangeHandlerLastTimer = performance.now();
  ...
  public onBeforeInputHandler(event: InputEvent): void {
    ...
    if (
      ...
      // set selection.
      this.scheduleOnDOMSelectionChangeFlush();
      this.onSelectionChangeHandlerFlush();
